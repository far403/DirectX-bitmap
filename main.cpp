...
bool Game_Init(HWND window)
{
	...
	if (!SUCCEEDED(result)){
		MessageBox(window,"Error on the SUCCEDED(result)","Error",MB_OK);
		return false;
	}
	...
	if (!SUCCEEDED(result)){
		...
		MessageBox(window,"File not loaded properly","ERROR",MB_OK|MB_ICONEXCLAMATION);
		return false;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (window == 0) {
		MessageBox(window,"Error Creating Window","Error",MB_OK);
		return 0;
	}
	...
	Game_Init(window);
	if (!Game_Init(window)){
		MessageBox(window,"Error Initializing the game","Error",MB_OK);
		return 0;
	}
